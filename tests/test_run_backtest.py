...
def test_parse_args_defaults():
    args = run_backtest.parse_args(["reliance", "--from", "2026-05-01", "--to", "2026-06-01"])
...
def test_parse_args_rejects_an_unsupported_interval():
    with pytest.raises(SystemExit):
        run_backtest.parse_args(
            ["RELIANCE", "--from", "2026-05-01", "--to", "2026-06-01", "--interval", "2hour"]
        )


def _candle(hour: int, minute: int, high: float, low: float, close: float, volume: int) -> Candle:
    return Candle(
...
def test_build_engine_composes_a_backtest_that_trades():
    ...
    args = run_backtest.parse_args(
        ["reliance", "--from", "2026-07-30", "--to", "2026-07-30", "--capital", "75000"]
    )
    engine, portfolio = run_backtest.build_engine(args)

    assert portfolio.cash == 75_000.0
    assert type(engine.runtime.execution_provider).__name__ == "SimulatedExecutionProvider"
    # A backtest must fail loudly rather than skip a bad candle.
    assert engine.runtime.config.continue_on_error is False

    engine.run(_one_breakout_session())

    assert len(portfolio.trade_log) == 1
    assert portfolio.trade_log.records[0].reason == "end_of_day_exit"


def _one_breakout_session() -> list[Candle]:
    """A session that warms every default indicator, then breaks out.

    ``IndicatorContext.with_defaults`` uses a 20-bar relative-volume
    lookback, so a breakout only clears the volume filter after twenty
    prior bars have been observed.
    """
    ...
    # Twenty quiet in-range bars from 9:30, to fill the volume lookback.
    minute = 30
    for _ in range(20):
        candles.append(_candle(9 + minute // 60, minute % 60, high=100, low=98, close=99, volume=100))
        minute += 5
    # Volume 300 against a 100-bar average clears the 2.0x filter, and the
    # close is above both the opening high (102) and VWAP (~99).
    candles.append(_candle(9 + minute // 60, minute % 60, high=110, low=103, close=108, volume=300))
    candles.append(_candle(15, 15, high=110, low=100, close=106, volume=100))
    return candles
...
def test_main_reports_auth_failure_without_traceback(monkeypatch, capsys):
    ...
    exit_code = run_backtest.main(["RELIANCE", "--from", "2026-05-01", "--to", "2026-06-01"])
...
def test_print_banner_is_ascii_safe_for_cp1252_consoles():
    ...
    args = run_backtest.parse_args(["RELIANCE", "--from", "2026-07-30", "--to", "2026-07-30"])
    buffer = io.TextIOWrapper(io.BytesIO(), encoding="cp1252", errors="strict")

    with contextlib.redirect_stdout(buffer):
        run_backtest.print_banner(args, [_candle(9, 15, 101, 90, 99, 100)])
    buffer.flush()
